...
// ================================================================
// ===              DELAYED CONFIRMED TOP                       ===
// ================================================================
bool topCandidateActive = false;
unsigned long topCandidateStart = 0;

unsigned long TOP_CONFIRM_MS = 120;   // confirmation window
float RESUME_THRESHOLD = 0.05;        // upward cancel threshold
...
void loop() {
  ...
  // ================================================================
  // SEARCHING FOR TOP (DELAYED CONFIRMATION ADDED)
  // ================================================================
  else if (currentState == SEARCH_TOP) {

    // ---------- START CANDIDATE ----------
    if (!topCandidateActive &&
        position > currentROMGate &&
        wasAscending &&
        velocity < -velocityDeadzone) {

      topCandidateActive = true;
      topCandidateStart = millis();

      Serial.println(">>> TOP CANDIDATE FOUND");
    }

    // ---------- CONFIRMATION WINDOW ----------
    if (topCandidateActive) {

      // Cancel if clear upward motion resumes
      if (velocity > RESUME_THRESHOLD) {
        Serial.println(">>> TOP CANDIDATE CANCELLED (Upward Resume)");
        topCandidateActive = false;
      }

      // After window expires → decide
      else if (millis() - topCandidateStart > TOP_CONFIRM_MS) {

...
void loop() {
  ...
  // ========================
  if (!orientationReady) {
    // Use a rough gravity estimate (magnitude of accel) to check stability
  ...
  // ========================
  float linearZ = restingGravity - az_world;  // Sign convention preserved
  linearZFiltered = linearZFiltered + alpha * (linearZ - linearZFiltered);
  float linearZOut = (abs(linearZFiltered) < deadZone) ? 0.0 : linearZFiltered;
  Serial.println(linearZOut);
}
...
void loop() {
  ...
  // --------------------------
  // VELOCITY (TRAPEZOIDAL) + ZUPT
  // --------------------------
  if (systemStill) {
    // While the device is still, we force everything to a clean zero.
    // This kills the "drift" that ruins VBT data.
    velocityZ = 0;
    prevLinearZ = 0; 
  } else {
    // When movement is detected, we integrate.
    // Starting from prevLinearZ = 0 ensures a smooth launch.
